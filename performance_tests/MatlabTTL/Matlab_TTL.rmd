...
Pckglst = c("ezknitr", "ggplot2", "plyr")
...
```{r, message=FALSE, warning=FALSE}
mu <- ddply(ON_dt, "Board", summarise, grp.mean=mean(ITV_diff, na.rm=TRUE))
...
ggplot(ON_dt, aes(x=ITV_diff, color=Board, fill=Board))                              +
    ...
    geom_vline(data=mu, aes(xintercept=grp.mean, color=Board), linetype="dashed")    +
...
```{r, message=FALSE, warning=FALSE}
mu <- ddply(OFF_dt, "Board", summarise, grp.mean=mean(ITV_diff, na.rm=TRUE))
